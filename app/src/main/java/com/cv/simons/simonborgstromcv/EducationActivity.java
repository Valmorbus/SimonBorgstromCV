...
public class EducationActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ArrayList<Education> educationList = new ArrayList<Education>();
        ...
        try {
            ...
            for (int i = 0; i < m_jArry.length(); i++) {
                ...
                educationList.add(new Education(jo_inside.getString("Time"), jo_inside.getString("Name"), jo_inside.getString("Description")));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }

        ArrayAdapter<Education> arrayAdapter = new ArrayAdapter<>(EducationActivity.this,
                android.R.layout.simple_list_item_1, android.R.id.text1, educationList);

        ListView eListView = (ListView) findViewById(R.id.listView2);
        eListView.setAdapter(arrayAdapter);

        eListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent i = new Intent(EducationActivity.this, DetailActivity.class);
                i.putExtra("qualifier", "education");
                i.putExtra("educationTime", educationList.get(position).getTime());
                i.putExtra("educationTitle", educationList.get(position).getName());
                i.putExtra("educationDescription", educationList.get(position).getDescription());
                startActivity(i);
            }
        });
    ...
    private String loadJSONFromAsset() {
        String json = null;
        try {
            InputStream is = this.getAssets().open("GetEducation.json");
